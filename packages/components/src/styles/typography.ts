...
export const typography = {
  body: css(
    {
      fontSize: theme.utils.pixel(bodyFontSize.base),
      fontWeight: theme.typography.weight.normal,
      ...Object.entries(bodyFontSize).reduce<CustomCSS>((acc, [size, fontSize]) => {
        acc[theme.attr.create('font', 'size').eq(size)] = { fontSize: theme.utils.pixel(fontSize) }
        return acc
      }, {}),
  ...
  header: css(
    {
      fontSize: theme.utils.pixel(headerFontSize.base),
      fontWeight: theme.typography.weight.strong,
      ...Object.entries(headerFontSize).reduce<CustomCSS>((acc, [key, fontSize]) => {
        acc[theme.attr.create('font', 'size').eq(key)] = { fontSize: theme.utils.pixel(fontSize) }
        return acc
      }, {}),
    },
    FONT_WEIGHT,
